...
### 🟡 Phase 3: 전역 실시간 파이프라인 & 디스코드 스타일 2단 사이드바 레이아웃 (Completed)
...
#### **Issue #117: [MOBILE-3] 핵심 대시보드·음성채널·설정 화면 모바일/태블릿 레이아웃 붕괴**
- **상태**: ✅ 완료 (Resolved)
- **설명**: 390px에서 설정 사이드바가 본문 폭을 거의 전부 차지하고, 친구/DM·빈 화면에서 닫힌 탐색을 다시 열 수 없으며, 채널 헤더·VoiceGrid 컨트롤이 충돌하던 문제를 수정. SettingsModal을 모바일 가로 탭 + `100dvh` 스크롤 구조로 전환하고, 모든 핵심 화면에 탐색 복귀 버튼·헤더 줄바꿈/말줄임·VoiceGrid 적응형 열/높이·모바일 팝오버 폭 제한을 적용. 390×844, 768×1024(사이드바 포함), 1024×768 실제 브라우저에서 가로 오버플로 없음 확인 및 `tests/e2e/responsive-core.spec.ts` 추가.
