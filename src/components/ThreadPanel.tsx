          ...
          {!thread ? <p className="text-sm text-zinc-500">{t("common.loading")}</p> : (
            <>
              <article className="mb-4 border-b border-white/10 pb-4" data-testid="thread-root">
                ...
                <p className={`mt-1 whitespace-pre-wrap text-sm ${thread.root.deletedAt ? "italic text-zinc-500" : "text-zinc-200"}`}>{body(thread.root)}</p>
              </article>
              <p className="mb-2 text-xs font-semibold text-zinc-500">{thread.replies.length} replies</p>
              <div className="space-y-3">
                {thread.replies.map((reply) => (
                  <article key={reply.id} data-testid="thread-reply" className="rounded-xl bg-white/[0.04] p-3">
                    ...
                    {editingId === reply.id ? (
                      ...
                      }} className="mt-2 w-full rounded-lg border border-zinc-700 bg-zinc-900 px-3 py-2 text-sm outline-hidden" />
                    ) : <p className={`mt-1 whitespace-pre-wrap text-sm ${reply.deletedAt ? "italic text-zinc-500" : "text-zinc-200"}`}>{body(reply)}</p>}
                  </article>
                ))}
              </div>
            </>
